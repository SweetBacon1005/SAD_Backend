...
export class ProductService {
  ...
  async getRecommendProducts(
    ...
    const response = await axios.get(`http://127.0.0.1:5000/recommend?user_id=${id}`);
    ...
    const [recommendProducts, popularProducts] = await Promise.all([
      recommendIds.length > 0
      ? this.prisma.product.findMany({
          where: { id: { in: recommendIds } },
          include: { category: true },
        })
      : Promise.resolve([]),  // Không gọi DB nếu mảng rỗng
    ...
    return {
      recommend: this.mapProductsToResponse(recommendProducts),
      popular: this.mapProductsToResponse(popularProducts),
    }
  }
  ...
  async compareProducts(
    ...
    const features = [
      ...
      },
      {
        name: 'Danh mục',
        values: products.map(
          (p) => p.category?.name || 'Không có thông tin',
        ),
      },
    ];
