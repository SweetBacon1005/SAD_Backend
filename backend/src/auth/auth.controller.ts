...
import { SendEmailVerificationOtpDto, VerifyEmailOtpDto } from './dto/verify-email-otp.dto';
import {
...
export class AuthController {
  ...
  @ApiResponse({ status: 400, description: 'Email không tồn tại' })
  async sendEmailVerificationOTP(@Body() payload: SendEmailVerificationOtpDto): Promise<void> {
  ...
  @ApiResponse({ status: 400, description: 'Email không tồn tại' })
  async sendPasswordResetOTP(@Body() payload: SendPasswordResetOtpDto): Promise<void> {
  ...
  @ApiResponse({ status: 200, description: 'Xác thực mã OTP thành công', type: AuthResponseDto })
  @ApiResponse({ status: 400, description: 'Mã OTP không hợp lệ hoặc đã hết hạn' })
  async verifyPasswordResetOTP(@Body() payload: VerifyPasswordResetOtpDto): Promise<AuthResponseDto> {
